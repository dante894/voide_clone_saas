"""
Servidor de generación de audio (XTTS-v2) para correr en Google Cloud Run.
...
Así, la parte pesada (PyTorch + XTTS-v2) corre acá, donde se le puede dar
toda la RAM que haga falta, y Render se queda liviano.

Endpoint:
    POST /generate
    Headers: Authorization: Bearer <VOICE_ENGINE_SECRET>
    Form-data:
        text: str
    ...
    secreto, sirve para verificar que el servicio está despierto.
"""
...
API_SECRET = os.environ.get("VOICE_ENGINE_SECRET", "")
...
def get_model():
    ...
    with _model_lock:
        if _model is None:
            ...
            print(f"[cloud-run] Cargando modelo XTTS-v2 en: {device} ...")
            _model = TTS("tts_models/multilingual/multi-dataset/xtts_v2").to(device)
            print("[cloud-run] Modelo listo.")
        return _model
